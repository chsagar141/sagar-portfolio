    ...
    <section className="py-24 px-8 border-t border-white/5 relative z-10">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center space-x-4 mb-16">
          ...
          <span className="text-[10px] uppercase tracking-[0.3em] font-bold opacity-40">Experience</span>
        </div>
        ...
        <div className="space-y-4">
          {experiences.map((exp, index) => (
            <motion.div 
              ...
              className="border border-white/5 bg-[#080808] p-8 md:p-12"
            >
              <div className="md:grid md:grid-cols-[1fr_3fr] md:gap-12 items-start">
                ...
                <div>
                  <h4 className="text-2xl font-light tracking-tight text-white mb-6 leading-tight">{exp.role}</h4>
                  <ul className="space-y-4">
                    {exp.description.map((item, i) => (
                      <li key={i} className="text-[13px] opacity-70 font-light leading-relaxed flex items-start">
                        <span className={`mr-4 mt-1.5 block w-1 h-1 bg-white/20 shrink-0`} />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
