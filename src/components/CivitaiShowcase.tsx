...
export function CivitaiShowcase() {
  ...
  if (loading) {
    return (
      <section className="py-24 px-8 border-t border-white/5 relative z-10 bg-[#050505]/40 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center space-x-4 mb-16">
            <span className="text-[10px] uppercase tracking-[0.3em] font-bold opacity-40">Civitai Models</span>
            <div className="flex-1 h-[1px] bg-white/5"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">
            {[1, 2, 3].map((i) => (
              <div key={i} className="aspect-[4/5] bg-[#0a0a0a] animate-pulse border border-white/5"></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto">
          ...
          {models.map((model, index) => {
            ...
            return (
              <motion.a
                ...
                className="group relative block aspect-[4/5] bg-[#080808] border border-white/5 overflow-hidden"
              >
                {/* Background Image */}
                {imageUrl ? (
                  <img
                  ...
                  <div className="absolute inset-0 bg-[#0a0a0a] flex items-center justify-center">
                    <span className="text-[10px] uppercase tracking-widest text-white/20">No Image Preview</span>
                  </div>
                )}

                <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/40 to-transparent opacity-80 group-hover:opacity-100 transition-opacity duration-500"></div>
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                  <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    ...
                    <h4 className="text-lg font-light tracking-tight text-white mb-2 line-clamp-2 leading-snug">
                      {model.name}
                    </h4>

                    <div className="flex items-center space-x-2 text-[9px] uppercase tracking-widest text-indigo-400 font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
        ...
        <div className="mt-12 text-center">
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="inline-flex items-center space-x-2 px-8 py-3 border border-white/10 hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all text-[10px] tracking-[0.2em] uppercase text-white/60 hover:text-white"
          >
